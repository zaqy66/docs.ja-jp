...
* [`deterministic`コンパイラ オプション](#deterministic-compiler-output):
    - 決定論的コンパイラ オプションは、同じソースの次回以降のコンパイルでも同じバイナリ出力を生成することが確定します。
...
このクラスが大きくなってくると、他のコンス トラクターが追加されることもあるでしょう。 各コンス トラクターでは、Grades プロパティを初期化する必要があります。そうしないとエラーが発生します。
...
プロパティ初期化子は、前述のように読み取り専用プロパティと、ここで示すように読み取り/書き込みプロパティで使用できます。
