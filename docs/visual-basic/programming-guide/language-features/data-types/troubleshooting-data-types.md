...
## <a name="string-conversion-fails-at-run-time"></a>String の変換は実行時に失敗します。  
 [String データ型](../../../../visual-basic/language-reference/data-types/string-data-type.md)は、ごく少数の拡大変換に関与します。 `String`はそれ自体と`Object`にのみ広がり、`Char`と`Char()`(、`Char`配列) は`String`に広がります。 これは、データ型と定数には、他のデータ型には含めることができない値を含めることができるためです。 
