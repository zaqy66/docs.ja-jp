 ...
 **✓ DO** 破棄可能な型のインスタンスを含む型で、基本的な Dispose パターンを実装します。 参照してください、 [Dispose の基本的なパターン](#basic_pattern)基本的なパターンの詳細セクションです。  
 ...
 **✓ DO** を許可する、`Dispose(bool)`に複数回呼び出されるメソッド。 メソッドは、最初の呼び出し後に何もすることもできます。  
