 ...
 **✓ CONSIDER** 要素の数が少ない配列を渡すエンドユーザーが予想される場合、配列パラメーターに、params キーワードを追加します。 多数の要素が渡されることに共通のシナリオと予測される場合は、ユーザーが、これらの要素のインラインを渡す可能性がありますし、ので、params キーワードは必要はありません。  
