 ...
 **✓ DO** 適用、<xref:System.FlagsAttribute?displayProperty=nameWithType>フラグ列挙体にします。 単純な列挙体にはこの属性は適用されません。  
 ...
 **✓ DO** フラグ列挙型のゼロ値の名前を付けます`None`です。 場合は、フラグ列挙型値必要があります常に意味「すべてのフラグはクリアされます」  
