...
Products テーブルは Catalog マイクロサービスによって所有されているので、Ordering マイクロサービスが Products テーブルを直接更新することはできません。 Ordering マイクロサービスが Catalog マイクロサービスを更新するには、統合イベント (メッセージとイベント ベースの通信) などの非同期通信のみを使う必要があります。 これは、[eShopOnContainers](https://aka.ms/eshoponcontainers) 参照アプリケーションがこの種類の更新を実行している方法です。
