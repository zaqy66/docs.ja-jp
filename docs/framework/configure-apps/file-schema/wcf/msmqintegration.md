...
|deadLetterQueue|使用する配信不能キューの種類を指定します。<br /><br /> 有効な値を次に示します。<br /><br /> カスタム: カスタム配信不能キュー。<br />-None: 配信不能キューが使用されます。<br />-システムの場合は、システム配信不能キューを使用します。<br /><br /> この属性は、DeadLetterQueue 型です。|  
...
|serializationFormat|MSMQ メッセージの一部として送信されるオブジェクトのシリアル化に使用されるフォーマッタを指定します。 有効な値は、次のとおりです。<br /><br /> ActiveX: COM オブジェクトをシリアル化すると、ActiveX フォーマッタが使用されます。<br />: バイナリでは、バイナリ パケットにオブジェクトをシリアル化します。<br />-ByteArray:、バイトの配列にオブジェクトがシリアル化します。<br />-Stream:、ストリームにオブジェクトがシリアル化します。<br />Xml: XML パケットにオブジェクトをシリアル化します。 既定値は XML です。<br /><br /> この属性は <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat> 型です。|  
