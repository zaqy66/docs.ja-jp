...
-   **サービス**: 探索プロキシに自らを登録する、単純な電卓の Windows Communication Foundation (WCF) サービス。  
  
-   **探索プロキシ**: 探索プロキシ サービスの実装。  
  
-   **クライアント**: する WCF クライアント アプリケーションを検索する探索プロキシ サービスを呼び出して取得します。  
