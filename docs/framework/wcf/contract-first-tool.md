...
-   **シリアライザー モード**: この設定を決定するシリアライザーは、サービス コントラクト ファイルの読み取りに使用されます。 ときに**XML シリアライザー**が選択されている、**コレクション型**と**型の再利用**オプションが無効になります。 これらのオプションにのみ適用されます、**データ コントラクト シリアライザー**します。

-   **型を再利用**: この設定は、型の再利用するために使用されるライブラリを指定します。 この設定は、場合にのみ適用されます。**シリアライザー モード**に設定されている**データ コントラクト シリアライザー**します。

-   **コレクション型**: この設定は、コレクションのデータ型に使用する完全修飾またはアセンブリ修飾型を指定します。 この設定は、場合にのみ適用されます。**シリアライザー モード**に設定されている**データ コントラクト シリアライザー**します。
...
-   **[Supportfx35typeddatasets]**: この設定では、.Net 用に作成された、型指定されたデータ セットの追加機能を提供するかどうかを指定します Framework 3.5。 ときに**シリアライザー モード**に設定されている**XML シリアライザー**、<xref:System.Data.Design.TypedDataSetSchemaImporterExtensionFx35>拡張機能は、この値が True に設定されている場合は、XML スキーマ インポーターに追加されます。 ときに**シリアライザー モード**に設定されている**データ コントラクト シリアライザー**、種類<xref:System.DateTimeOffset>この値が False に設定されている場合に、参照から除外するように、<xref:System.DateTimeOffset>常に生成されます古いバージョンのフレームワークです。
