...
---
# <a name="annotations-schema"></a><span data-ttu-id="e74bb-102">注釈スキーマ</span><span class="sxs-lookup"><span data-stu-id="e74bb-102">Annotations Schema</span></span>
<span data-ttu-id="e74bb-103">このトピックでは、Microsoft Annotations Framework でユーザー注釈データの保存と取得に使用される XML スキーマ定義 (XSD) について説明します。</span><span class="sxs-lookup"><span data-stu-id="e74bb-103">This topic describes the XML schema definition (XSD) used by the Microsoft Annotations Framework to save and retrieve user annotation data.</span></span>  
  
 <span data-ttu-id="e74bb-104">[!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] は注釈データをシリアル化し、内部表現を XML 形式に変換します。</span><span class="sxs-lookup"><span data-stu-id="e74bb-104">The [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] serializes annotation data from an internal representation to an XML format.</span></span>  <span data-ttu-id="e74bb-105">この変換に使用される XML 形式は [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] XSD スキーマで表現されます。</span><span class="sxs-lookup"><span data-stu-id="e74bb-105">The XML format used for this conversion is described by the [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] XSD Schema.</span></span>  <span data-ttu-id="e74bb-106">このスキーマは実装非依存の XML 形式を定義します。実装非依存の XML 形式を利用し、アプリケーション間で注釈データを交換できます。</span><span class="sxs-lookup"><span data-stu-id="e74bb-106">The schema defines the implementation-independent XML format that can be used to exchange annotation data between applications.</span></span>  
  
 <span data-ttu-id="e74bb-107">[!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] XML スキーマ定義は 2 つのサブスキーマから構成されています。</span><span class="sxs-lookup"><span data-stu-id="e74bb-107">The [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] XML schema definition consists of two subschemas</span></span>  
  
-   <span data-ttu-id="e74bb-108">注釈 XML コア スキーマ (コア スキーマ)。</span><span class="sxs-lookup"><span data-stu-id="e74bb-108">The Annotations XML Core Schema (Core Schema).</span></span>  
  
-   <span data-ttu-id="e74bb-109">注釈 XML 基本スキーマ (基本スキーマ)。</span><span class="sxs-lookup"><span data-stu-id="e74bb-109">The Annotations XML Base Schema (Base Schema).</span></span>  
  
 <span data-ttu-id="e74bb-110">コア スキーマは、のプライマリ XML 構造を定義、<xref:System.Windows.Annotations.Annotation>します。</span><span class="sxs-lookup"><span data-stu-id="e74bb-110">The Core Schema defines the primary XML structure of an <xref:System.Windows.Annotations.Annotation>.</span></span>  <span data-ttu-id="e74bb-111">コア スキーマで定義された XML 要素の大部分が型に対応、<xref:System.Windows.Annotations>名前空間。</span><span class="sxs-lookup"><span data-stu-id="e74bb-111">The majority of XML elements defined in the Core Schema correspond to types in the <xref:System.Windows.Annotations> namespace.</span></span>  <span data-ttu-id="e74bb-112">コア スキーマは 3 つの拡張ポイントを公開します。そのポイントでアプリケーションは独自の XML データを追加できます。</span><span class="sxs-lookup"><span data-stu-id="e74bb-112">The Core Schema exposes three extension points where applications can add their own XML data.</span></span>  <span data-ttu-id="e74bb-113">これらの拡張ポイントを含める、 <xref:System.Windows.Annotations.Annotation.Authors%2A>、 <xref:System.Windows.Annotations.ContentLocatorPart>、「コンテンツ」とします。</span><span class="sxs-lookup"><span data-stu-id="e74bb-113">These extension points include the <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and "Content".</span></span>  <span data-ttu-id="e74bb-114">(要素がの形式で提供されるコンテンツ、<xref:System.Xml.XmlElement>一覧です)。</span><span class="sxs-lookup"><span data-stu-id="e74bb-114">(Content elements are provided in the form of an <xref:System.Xml.XmlElement> list.)</span></span>  
  
 <span data-ttu-id="e74bb-115">このトピックで説明されているベース スキーマの拡張機能を定義する、 <xref:System.Windows.Annotations.Annotation.Authors%2A>、 <xref:System.Windows.Annotations.ContentLocatorPart>、コンテンツの種類の初期の Windows Presentation Foundation (WPF) のリリースに含まれているとします。</span><span class="sxs-lookup"><span data-stu-id="e74bb-115">The Base Schema described in this topic defines the extensions for the <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and Content types included with the initial Windows Presentation Foundation (WPF) release.</span></span>  
  
<a name="CoreSchema"></a>   
## <a name="annotations-xml-core-schema"></a><span data-ttu-id="e74bb-116">注釈 XML コア スキーマ</span><span class="sxs-lookup"><span data-stu-id="e74bb-116">Annotations XML Core Schema</span></span>  
 <span data-ttu-id="e74bb-117">注釈 XML コア スキーマ定義を格納するために使用される XML 構造<xref:System.Windows.Annotations.Annotation>オブジェクト。</span><span class="sxs-lookup"><span data-stu-id="e74bb-117">The Annotations XML Core Schema defines the XML structure that is used to store <xref:System.Windows.Annotations.Annotation> objects.</span></span>  
...
<a name="BaseSchema"></a>   
## <a name="annotations-xml-base-schema"></a><span data-ttu-id="e74bb-118">注釈 XML 基本スキーマ</span><span class="sxs-lookup"><span data-stu-id="e74bb-118">Annotations XML Base Schema</span></span>  
 <span data-ttu-id="e74bb-119">基本スキーマは、コア スキーマで定義されている 3 つの抽象要素の XML 構造を定義します<xref:System.Windows.Annotations.Annotation.Authors%2A>、 <xref:System.Windows.Annotations.ContentLocatorPart>、および<xref:System.Windows.Annotations.AnnotationResource.Contents%2A>します。</span><span class="sxs-lookup"><span data-stu-id="e74bb-119">The Base Schema defines the XML structure for the three abstract elements defined in the Core Schema – <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and <xref:System.Windows.Annotations.AnnotationResource.Contents%2A>.</span></span>  
...
<a name="SampleXML"></a>   
## <a name="sample-xml-produced-by-annotations-xmlstreamstore"></a><span data-ttu-id="e74bb-120">注釈 XmlStreamStore で作成されたサンプル XML</span><span class="sxs-lookup"><span data-stu-id="e74bb-120">Sample XML Produced by Annotations XmlStreamStore</span></span>  
 <span data-ttu-id="e74bb-121">続く XML では、注釈の出力を示しています。<xref:System.Windows.Annotations.Storage.XmlStreamStore>と 3 つの注釈の強調表示や、テキスト付箋、インク付箋を含むサンプル ファイルの組織。</span><span class="sxs-lookup"><span data-stu-id="e74bb-121">The XML that follows shows the output of an Annotations <xref:System.Windows.Annotations.Storage.XmlStreamStore> and the organization of a sample file that contains three annotations - a highlight, a text sticky-note, and an ink stick-note.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="e74bb-122">関連項目</span><span class="sxs-lookup"><span data-stu-id="e74bb-122">See Also</span></span>  
 ...
 [<span data-ttu-id="e74bb-123">注釈の概要</span><span class="sxs-lookup"><span data-stu-id="e74bb-123">Annotations Overview</span></span>](../../../../docs/framework/wpf/advanced/annotations-overview.md)
